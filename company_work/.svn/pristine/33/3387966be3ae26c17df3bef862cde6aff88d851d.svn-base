<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="ahu6List" @selection-change="handleSelectionChange">
...
<script>
import { listAhu6, getAhu6, delAhu6, addAhu6, updateAhu6 } from "@/api/modbus/ahu6";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listAhu6(this.queryParams).then(response => {
        this.ahu6List = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.rowId != null) {
            updateAhu6(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addAhu6(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('/modbus/ahu6/export', {
        ...this.queryParams
      }, `ahu6_${new Date().getTime()}.xlsx`)
    },
    utcFormat(dateStr) {
