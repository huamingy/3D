<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="ahu5List" @selection-change="handleSelectionChange">
...
<script>
import { listAhu5, getAhu5, delAhu5, addAhu5, updateAhu5 } from "@/api/modbus/ahu5";
...
export default {
  ...
  data() {
    return {
      ...
      // 是否显示弹出层
      open: false,
      exportUrl: '/modbus/ahu5/export',
      exportName: 'ahu5_',
      modbusType: 'ModbusAhu5',
  ...
  methods: {
    ...
    getList() {
      ...
      listAhu5(this.queryParams).then(response => {
        this.ahu5List = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.rowId != null) {
            updateAhu5(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addAhu5(this.form).then(response => {
