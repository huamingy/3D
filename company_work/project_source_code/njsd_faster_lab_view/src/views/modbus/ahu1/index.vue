<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="ahu1List" @selection-change="handleSelectionChange">
...
<script>
import { listAhu1, getAhu1, delAhu1, addAhu1, updateAhu1 } from "@/api/modbus/ahu1";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listAhu1(this.queryParams).then(response => {
        this.ahu1List = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.rowId != null) {
            updateAhu1(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addAhu1(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('/modbus/ahu1/export', {
        ...this.queryParams
      }, `ahu1_${new Date().getTime()}.xlsx`)
    },
    utcFormat(dateStr) {
