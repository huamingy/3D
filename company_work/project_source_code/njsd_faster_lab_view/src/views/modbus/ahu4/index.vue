<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="ahu4List" @selection-change="handleSelectionChange">
...
<script>
import { listAhu4, getAhu4, delAhu4, addAhu4, updateAhu4 } from "@/api/modbus/ahu4";
...
export default {
  ...
  data() {
    return {
      ...
      // 是否显示弹出层
      open: false,
      exportUrl: '/modbus/ahu4/export',
      exportName: 'ahu4_',
      modbusType: 'ModbusAhu4',
  ...
  methods: {
    ...
    getList() {
      ...
      listAhu4(this.queryParams).then(response => {
        this.ahu4List = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.rowId != null) {
            updateAhu4(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addAhu4(this.form).then(response => {
