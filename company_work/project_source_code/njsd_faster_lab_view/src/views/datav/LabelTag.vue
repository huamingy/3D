  ...
  <div class="label-tag">
    <template v-if="mergedConfig">
      <div class="label-item" v-for="(label, i) in mergedConfig.data" :key="label">
        <div :style="`background-color: ${mergedConfig.colors[i % mergedConfig.colors.length]};margin-right: 10px;`" />
        <span :style="`color: ${mergedConfig.colors[i % mergedConfig.colors.length]};`" >{{ label }}</span>
      </div>
    </template>
  </div>
</template>
  ...
  data () {
    return {
      defaultConfig: {
         ...
         */
        colors: ['#3de7c9', '#ffc530', '#ff3300', '#00baff']
      },
